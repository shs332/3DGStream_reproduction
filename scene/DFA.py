...
class DFA_dataset(Dataset):
    def __init__(
        ...
        # Change directory to the processed dataset
        self.dir_from = f"/data2/wlsgur4011/GESI/gsplat/data/DFA_processed/{object_name}/{frame_from}"
        self.dir_to = f"/data2/wlsgur4011/GESI/gsplat/data/DFA_processed/{object_name}/{frame_to}"
        cam_idx = int(cam_idx)
        ...
        self.focal = [fy, fx] # focal[0] = fl_y, focal[1] = fl_x, same for all images
        self.FovY = focal2fov(self.focal[0], height)
        self.FovX = focal2fov(self.focal[1], width)
        self.transform = transforms.ToTensor()
        
        self.image_paths, self.image_poses, self.cxs, self.cys = [], [], [], []
        # breakpoint()
        # 이미지 경로, 포즈, 시간 
        if split == "train":
            ...
            idx = 0
            # test images: except target view single image
    ...
    def __getitem__(self, index):
        with Image.open(self.image_paths[index]) as img:
            ...
            w2c, cx, cy, fx, fy = self.image_poses[index], self.cxs[index], self.cys[index], self.focal[1], self.focal[0]
